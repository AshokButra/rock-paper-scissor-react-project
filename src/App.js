...
class App extends Component {
  state = {score: 0, randomChoosenObject: '', idValue: '', gamingView: true}

  onClickButton = id => {
    const randomGeneratedValue = Math.ceil(Math.random() * 3)
    if (randomGeneratedValue === 1) {
      this.setState({
        randomChoosenObject: 'ROCK',
        idValue: id,
        gamingView: false,
      })
    } else if (randomGeneratedValue === 2) {
      this.setState({
        randomChoosenObject: 'SCISSORS',
        idValue: id,
        gamingView: false,
      })
    } else {
      this.setState({
        randomChoosenObject: 'PAPER',
        idValue: id,
        gamingView: false,
      })
    }
        ...
        <div className="buttons-container">
          {choicesList.map(eachObject => (
            <button
              ...
              type="button"
              key={eachObject.id}
              data-testid={`${eachObject.id.lower()}Button`}
  ...
  renderResultsPage = () => {
    const {idValue, randomChoosenObject, score} = this.state
    ...
    const randomObject = choicesList.filter(
      eachObject => eachObject.id === randomChoosenObject,
    )
    ...
    return (
      <div className="bg-container-result">
        ...
        <div className="selected-random-choosen-container">
          <div className="you-selected-container">
            <p className="you">YOU</p>
            <img src={selectedImage} className="selected-image" alt={idValue} />
          </div>
          <div className="random-selected-container">
            ...
            <img
              ...
              alt={randomChoosenObject}
            />
          </div>
        </div>
        <GameResultsView
          idValue={idValue}
          randomChoosenObject={randomChoosenObject}
